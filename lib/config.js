...
function initialize(dimension, overrides) {
  var options = {
    // include a reference to this function so we can re-initialize the options
    ...
    // I originally wrote all of this with the width/height of 600; therefore
    // we need to scale all of the line widths, sizes, etc. to compensate for
    // differing dimensions
    scaleFactor: dimension / 600,

    // 8-bit mode
    pixelate: _.random(0, 100) >= 95,
    pixelateResolution: 32,
  ...
  options = _.assignIn({}, options, overrides);
